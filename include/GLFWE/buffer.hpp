...
public:
    ...
    Buffer && buffer_data(GLenum buffer_type, std::vector<T> & data, GLenum access_type) {
        return buffer_data(buffer_type, sizeof(T) * data.size(), data.data(), access_type);
    }

    template<typename T>
    Buffer && buffer_data(GLenum buffer_type, T & data, GLenum access_type) {
        return buffer_data(buffer_type, sizeof(T), data, access_type);
    }
    ...
    Buffer && buffer_sub_data(GLenum buffer_type, unsigned int offset, const std::vector<T> & data) {
        return buffer_sub_data(buffer_type, offset, sizeof(T) * data.size(), data.data());
    }
    template<typename T>
